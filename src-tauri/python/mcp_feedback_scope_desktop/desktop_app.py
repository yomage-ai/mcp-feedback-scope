...
try:
    from mcp_feedback_scope.debug import server_debug_log as debug_log
    from mcp_feedback_scope.web.main import WebUIManager, get_web_ui_manager
...
class DesktopApp:
    ...
    async def launch_tauri_app(self, server_url: str):
        ...
        try:
            from mcp_feedback_scope.desktop_release import __file__ as desktop_init
            ...
            # 定義平台到二進制文件的映射
            if system == "windows":
                tauri_exe = desktop_dir / "mcp-feedback-scope-desktop.exe"
            elif system == "darwin":  # macOS
                # 檢測 Apple Silicon 或 Intel
                if machine in ["arm64", "aarch64"]:
                    tauri_exe = (
                        desktop_dir / "mcp-feedback-scope-desktop-macos-arm64"
                    )
                else:
                    tauri_exe = (
                        desktop_dir / "mcp-feedback-scope-desktop-macos-intel"
                    )
            elif system == "linux":
                tauri_exe = desktop_dir / "mcp-feedback-scope-desktop-linux"
            else:
                # 回退到通用名稱
                tauri_exe = desktop_dir / "mcp-feedback-scope-desktop"
            ...
            if tauri_exe.exists():
                ...
                # 嘗試回退選項
                fallback_files = [
                    desktop_dir / "mcp-feedback-scope-desktop.exe",
                    desktop_dir / "mcp-feedback-scope-desktop-macos-intel",
                    desktop_dir / "mcp-feedback-scope-desktop-macos-arm64",
                    desktop_dir / "mcp-feedback-scope-desktop-linux",
                    desktop_dir / "mcp-feedback-scope-desktop",
                ]
            ...
            tauri_exe = (
                ...
                / "target"
                / "debug"
                / "mcp-feedback-scope-desktop.exe"
            )
